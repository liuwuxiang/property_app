...
function getMobileCode(){
    ...
    $.ajax({
        ...
        type:"POST",
        dataType : 'json',
        data:{"business_id":storage["business_id"],"type":9},
...
function setLoginPwd() {
    ...
    if (new_pwd == undefined || new_pwd == ""){
        ...
        toast(2,"开启loading");
        $.ajax({
            url:"/property_system/wnk_business_app/v1.0.0/setLoginPwd",
            type:"POST",
            dataType : 'json',
            data:{"business_id":storage["business_id"],"new_login_pwd":confirm_new_pwd,"code":code},
            success:function(data){
                toast(1,data.msg);
                if (data.status == 0){
