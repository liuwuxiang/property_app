$(function () {
    // 加载完成时候进行页面初始化
    $.ajax({
        url: '/property_system/wnk_business/getIntegralGoodsAll',
        type: 'post',
        dataType: 'json',
        timeout: 5000,
                ...
                if (parseInt(data.data.length) === 0) {
                    ...
                    data.data.forEach(function (ret) {
                        if (parseInt(ret.is_checked) === 0) {
                            yes.append('<a href="/property_system/wnk_business/joinIntegralGoodsEdit?goods_id=' + ret.id + '" class="item member A'+ret.name+'">' + ret.name + '</a>');
                            yes.append('<style>.A' + ret.name + ':before{background-image: url(' + data.msg + ret.img + ') !important;border-radius:50%;}</style>');
                        } else {
                            no.append( '<a href="/property_system/wnk_business/joinIntegralGoodsEdit?goods_id=' + ret.id + '" class="item member A'+ret.name+'">' + ret.name + '</a>');
