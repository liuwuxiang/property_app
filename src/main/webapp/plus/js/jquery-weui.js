...
+ function($) {
  ...
  $.prompt = function(text, title, onOK, onCancel, input) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="password" class="weui-input weui-prompt-input" id="weui-prompt-input" value="' + (config.input || '') + '" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
...
+ function($) {
  ...
  defaults = $.fn.datetimePicker.prototype.defaults = {
    ...
    years: (function () {
      var arr = [];
      for (var i = 1950; i <= 2130; i++) { arr.push(i); }
      return arr;
    })(),
    times: function () {
      return [  // 自定义的时间
