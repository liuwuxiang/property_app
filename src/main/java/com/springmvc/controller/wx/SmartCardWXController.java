...
public class SmartCardWXController {
    @Resource
    private LoginSessionIdsService sessionIdsService;

    //进入智能卡片主页
    @RequestMapping(value = "/joinSmartCard")
    @ResponseBody
    public ModelAndView joinSmartCard(HttpServletRequest request){
        ...
        if (isLogin == false){
            ...
            modelAndView = new ModelAndView("wx/property_center/smart_card");
        }
        return modelAndView;
    ...
    //进入添加智能卡片页
    @RequestMapping(value = "/joinAddSmartCard")
    @ResponseBody
    public ModelAndView joinAddSmartCard(HttpServletRequest request){
        ModelAndView modelAndView;
        CookileUtil cookileUtil = new CookileUtil();
        String user_id_str = cookileUtil.getCookie_user_id(request,2);
        //标记是否已登录(false-未登录,已登录)
        ...
        if (isLogin == false){
            ...
            modelAndView = new ModelAndView("wx/property_center/smart_card_distinguish");
        }
        return modelAndView;
