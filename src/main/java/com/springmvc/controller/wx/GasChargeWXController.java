...
public class GasChargeWXController {
    @Resource
    private LoginSessionIdsService sessionIdsService;

    //进入燃气费账单主页
    @RequestMapping(value = "/joinGasCharge")
    @ResponseBody
    public ModelAndView joinGasCharge(HttpServletRequest request){
        ...
        if (isLogin == false){
            ...
            modelAndView = new ModelAndView("wx/property_center/gas_charge");
        }
        return modelAndView;
    ...
    //进入燃气费账单详情页
    @RequestMapping(value = "/joinGasChargeDetail")
    @ResponseBody
    public ModelAndView joinGasChargeDetail(HttpServletRequest request){
        ModelAndView modelAndView;
        CookileUtil cookileUtil = new CookileUtil();
        String user_id_str = cookileUtil.getCookie_user_id(request,2);
        //标记是否已登录(false-未登录,已登录)
        ...
        if (isLogin == false){
            ...
            modelAndView = new ModelAndView("wx/property_center/gas_charge_detail");
        }
        return modelAndView;
