...
public class TVFeeWXController {
    @Resource
    private LoginSessionIdsService sessionIdsService;

    //进入电视费账单主页
    @RequestMapping(value = "/joinTVFee")
    @ResponseBody
    public ModelAndView joinTVFee(HttpServletRequest request){
        ...
        if (isLogin == false){
            ...
            modelAndView = new ModelAndView("wx/property_center/tv_fee");
        }
        return modelAndView;
    ...
    //进入电视费账单详情页
    @RequestMapping(value = "/joinTVFeeDetail")
    @ResponseBody
    public ModelAndView joinTVFeeDetail(HttpServletRequest request){
        ModelAndView modelAndView;
        CookileUtil cookileUtil = new CookileUtil();
        String user_id_str = cookileUtil.getCookie_user_id(request,2);
        //标记是否已登录(false-未登录,已登录)
        ...
        if (isLogin == false){
            ...
            modelAndView = new ModelAndView("wx/property_center/tv_fee_detail");
        }
        return modelAndView;
